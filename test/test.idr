...
main = do renderer <- SDL2.init 640 480
  ...
  where eventLoop : Renderer -> Integer -> Int -> Int -> Int -> Int -> IO ()
        ...
        processEvent r f x y mx my (Just (KeyDown (KeyAny k)))
                = do print k
                     eventLoop r f x y mx my
        processEvent r f x y mx my (Just (MouseMotion mousex mousey _ _))
                = do print (mousex, mousey)
                     eventLoop r f x y mx my
        processEvent r f x y mx my (Just (MouseButtonUp Left mousex mousey))
                = do print (mousex, mousey)
